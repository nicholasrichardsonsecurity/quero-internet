...
import { normalizedPaymentState, reconciliationInterval, retryDecision } from './reconciliation-policy.js';
...
assert.equal(normalizedPaymentState('PAYMENT_REFUNDED', 'REFUNDED'), 'REFUNDED');
assert.equal(reconciliationInterval(undefined), 60_000);
assert.equal(reconciliationInterval('5000'), 10_000);
