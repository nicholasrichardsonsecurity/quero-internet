...
export const BILLING_PRODUCT_KEYS = [
  'loopclub', 'meu-cond', 'taplink', 'quero-internet'
] as const;
...
export type AsaasWebhookPayload = {
  id?: unknown;
  event?: unknown;
  payment?: { id?: unknown; status?: unknown; externalReference?: unknown };
};
...
export function parseBillingReference(value: unknown): BillingReference | null {
  ...
  if ([productKey, tenantId, companyId, planId, paymentId].some((part) => !/^[A-Za-z0-9._-]{1,128}$/.test(part))) {
    return null;
  }
...
export function isBillingProductKey(value: unknown): value is BillingProductKey {
  return typeof value === 'string' && (BILLING_PRODUCT_KEYS as readonly string[]).includes(value);
}
