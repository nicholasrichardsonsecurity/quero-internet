...
import { ArrowLeft, CheckCircle2, Loader2, LogOut, RefreshCcw, ShieldCheck } from 'lucide-react';
...
export default function MunicipalBillingPage(){
  ...
  return <main className="billing-page"><div className="billing-top"><Link href="/" className="billing-back"><ArrowLeft size={16}/> Visão geral</Link><div className="billing-actions"><button className="billing-refresh" onClick={()=>void load()}><RefreshCcw size={15}/> Atualizar</button><button className="billing-logout" onClick={()=>void logout()}><LogOut size={15}/> Sair</button></div></div><section className="billing-hero municipal"><div><span className="eyebrow">FATURAMENTO OPERACIONAL • MUNICÍPIO</span><h1>Conferência de serviços</h1><p>Analise a medição enviada pelo provedor e aprove somente o valor efetivamente comprovado. A NF do provedor não é emitida pela Aplivora.</p></div><ShieldCheck size={42}/></section>{message?<div className="billing-message" role="status">{message}</div>:null}{loading?<div className="billing-empty"><Loader2 className="spin" size={20}/> Carregando dados reais...</div>:items.length===0?<div className="billing-empty">Nenhuma medição encontrada no contexto autorizado.</div>:<section className="billing-list">{items.map(item=><article className="billing-card" key={item.id}><div className="billing-card-head"><div><strong>Competência {date(item.competenceStart)} — {date(item.competenceEnd)}</strong><small>{item.beneficiaryCount} beneficiário(s) • {item.activeServiceCount} serviço(s)</small></div><span className={`status ${item.status.toLowerCase()}`}>{item.status}</span></div><div className="billing-meta"><span>Valor<strong>{money(item.totalAmount)}</strong></span><span>Enviada<strong>{date(item.submittedAt)}</strong></span><span>Aprovada<strong>{date(item.approvedAt)}</strong></span></div>{item.status==='SUBMITTED'?<button className="billing-action approve" disabled={busy===item.id} onClick={()=>void approve(item.id)}>{busy===item.id?<Loader2 className="spin" size={15}/>:<CheckCircle2 size={15}/>} Aprovar medição</button>:<div className="billing-note">Esta medição está em <strong>{item.status}</strong>.</div>}</article>)}</section>}</main>;
}
